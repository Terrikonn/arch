...
impl<'a, P: PageTableFrameMapping> MappedPageTable<'a, P> {
    ...
    /// `page_table_frame_mapping` closure is correct. Also, the passed `level_4_table` must
    /// point to the level 4 page table of a valid page table hierarchy. Otherwise this function
    /// might break memory safety, e.g. by writing to an illegal memory location.
        ...
        A: FrameAllocator<Size4KiB> + ?Sized,
    {
        ...
        let p3 = self.page_table_walker.create_next_table(&mut p4[page.p4_index()], parent_table_flags, allocator)?;
        ...
        A: FrameAllocator<Size4KiB> + ?Sized,
    {
        let p4 = &mut self.level_4_table;
        let p3 = self.page_table_walker.create_next_table(&mut p4[page.p4_index()], parent_table_flags, allocator)?;
        let p2 = self.page_table_walker.create_next_table(&mut p3[page.p3_index()], parent_table_flags, allocator)?;
...
impl<P: PageTableFrameMapping> PageTableWalker<P> {
    ...
    /// in the passed entry.
    fn next_table<'b>(&self, entry: &'b PageTableEntry) -> Result<&'b PageTable, PageTableWalkError> {
    ...
    /// `PageTableWalkError::MappedToHugePage` if the `HUGE_PAGE` flag is set
    /// in the passed entry.
    fn next_table_mut<'b>(&self, entry: &'b mut PageTableEntry) -> Result<&'b mut PageTable, PageTableWalkError> {
        let page_table_ptr = self.page_table_frame_mapping.frame_to_pointer(entry.frame()?);
