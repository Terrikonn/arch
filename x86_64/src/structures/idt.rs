...
pub struct InterruptDescriptorTable {
    ...
    /// - Execution of any of the following instructions in 64-bit mode: `AAA`, `AAD`, `AAM`, `AAS`,
    ///   `BOUND`, `CALL` (opcode 9A), `DAA`, `DAS`, `DEC`, `INC`, `INTO`, `JMP` (opcode EA), `LDS`,
    ///   `LES`, `POP` (`DS`, `ES`, `SS`), `POPA`, `PUSH` (`CS`, `DS`, `ES`, `SS`), `PUSHA`, `SALC`.
    /// - Execution of the `ARPL`, `LAR`, `LLDT`, `LSL`, `LTR`, `SLDT`, `STR`, `VERR`, or `VERW`
    ///   instructions when protected mode is not enabled, or when virtual-8086 mode is enabled.
    ...
    ///
    /// - Implied stack references in which the stack address is not in canonical form. Implied
    ///   stack references include all push and pop instructions, and any instruction using `RSP` or
    ///   `RBP` as a base register.
    ...
    ///   is not present in physical memory. This is indicated by a cleared present bit in the
    ...
    ///   both).
    /// - A reserved bit in one of the page-translation-table entries is set to 1. A `#PF` occurs
    ///   for this reason only when `CR4.PSE=1` or `CR4.PAE=1`.
    ///
    ...
    /// - Software executes an `INTn` instruction. The `INTn` instruction operand provides the
    ...
    ///   pointer points to the instruction immediately following the boundary where the external
...
impl InterruptDescriptorTable {
    ...
    ///
    /// Panics if range is outside the range of user interrupts (i.e. greater than 255) or if the
...
impl<F> Entry<F> {
    ...
    fn handler_addr(&self) -> u64 {
        self.pointer_low as u64 | (self.pointer_middle as u64) << 16 | (self.pointer_high as u64) << 32
    }
}
...
impl fmt::Debug for EntryOptions {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_tuple("EntryOptions").field(&format_args!("{:#06x}", self.0)).finish()
    }
}
