...
impl VersionInformation {
    ...
    /// if `VersionInformation::htt()` is true.
    ...
    /// This number is assigned to the local APIC on the processor during power
    /// up.
    ///
...
impl ExtendedTopologyLeaf {
    ...
    /// Number of bits to shift right on x2APIC ID to get a unique topology ID
    /// of the next level type.
    ///
    ...
    /// EBX[15:0], depending on software and platform hardware
    /// configurations.
...
impl Master {
    ...
    master_attr_reader!(
        processor_extended_state_secondary,
        ProcessorExtendedStateSecondary
    );
